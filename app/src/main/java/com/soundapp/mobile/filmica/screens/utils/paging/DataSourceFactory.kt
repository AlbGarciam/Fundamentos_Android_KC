...
class FilmsDataSourceFactory(val repository: DataSourceRepository<Film>,
                             val context: Context,
                             val onError: (Error?) -> Unit): DataSource.Factory<Int, Film>() {
    private lateinit var filmsDataSource: FilmsDataSource

    override fun create(): DataSource<Int, Film> {
        filmsDataSource = FilmsDataSource(repository, context, onError)
